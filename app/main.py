...
def setup_status():
    return {
        **svc.runtime_live_status(),
        'latest_official_dataset': {
            'name': '2026년_버스노선별_정류장별_시간대별_승하차_인원_정보(08월).csv',
            'published_or_modified': '2026-09-06',
            'size_mb': 15.24,
...
def direct_journey(req: DirectJourneyRequest):
    try:
        return svc.discovery.direct_candidates(
            origin_station_id=req.origin_station_id,
            origin_ars_id=req.origin_ars_id,
            destination_station_id=req.destination_station_id,
            destination_ars_id=req.destination_ars_id,
            hour=req.hour,
        )
    except Exception as e:
...
def transfer_journey(req: TransferJourneyRequest):
    try:
        return svc.transfer_discovery.transfer_candidates(
